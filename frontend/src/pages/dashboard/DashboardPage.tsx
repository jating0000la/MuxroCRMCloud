...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      cardBg: 'from-indigo-50 to-violet-50',
  ...
  return (
    <Layout>
      <div className="sleek-page min-h-screen bg-[radial-gradient(circle_at_top_left,_#dbeafe_0%,_transparent_35%),radial-gradient(circle_at_top_right,_#fce7f3_0%,_transparent_35%),radial-gradient(circle_at_bottom_left,_#dcfce7_0%,_transparent_30%),linear-gradient(to_bottom,_#f8fafc,_#ffffff)]">
        <div className="max-w-7xl mx-auto p-4 lg:p-6 space-y-4">
          <section className="rounded-2xl border border-cyan-100 bg-gradient-to-r from-cyan-50/90 via-sky-50/90 to-blue-50/90 backdrop-blur px-4 py-4 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.14em] text-cyan-700 font-bold">Daily Control Panel</p>
                <p className="mt-1 text-sm text-slate-700 font-medium">
                  {format(new Date(), 'EEEE, MMMM d, yyyy')} • {roleLabel}
                </p>
                {lastUpdated && (
                  <p className="text-xs text-slate-500 mt-1">Last updated: {format(lastUpdated, 'MMM d, yyyy h:mm a')}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 w-full lg:w-auto">
                <div className="min-w-0 sm:col-span-2 lg:col-span-1 lg:w-64">
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200 disabled:bg-slate-100"
                  >
                ...
                <input
                  ...
                  className="rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200"
                />
                ...
                <button
                  onClick={loadData}
                  disabled={refreshing}
                  className="inline-flex items-center justify-center rounded-lg border border-cyan-200 bg-white px-3.5 py-2 text-xs font-semibold text-cyan-800 hover:bg-cyan-50 disabled:opacity-60"
                >
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <button onClick={() => applyDatePreset('today')} className="px-2.5 py-1 text-xs font-semibold border border-cyan-200 rounded-md bg-white text-cyan-700 hover:bg-cyan-50">Today</button>
              <button onClick={() => applyDatePreset('7d')} className="px-2.5 py-1 text-xs font-semibold border border-cyan-200 rounded-md bg-white text-cyan-700 hover:bg-cyan-50">7D</button>
              <button onClick={() => applyDatePreset('30d')} className="px-2.5 py-1 text-xs font-semibold border border-cyan-200 rounded-md bg-white text-cyan-700 hover:bg-cyan-50">30D</button>
              <button onClick={() => applyDatePreset('month')} className="px-2.5 py-1 text-xs font-semibold border border-cyan-200 rounded-md bg-white text-cyan-700 hover:bg-cyan-50">This Month</button>
            </div>
          </section>
          ...
          {analyticsError && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 flex items-center justify-between gap-3">
              ...
              <button onClick={loadUserReport} className="rounded-lg bg-amber-200 px-3 py-1 text-xs font-semibold text-amber-900 hover:bg-amber-300">
                Retry
              </button>
            </div>
          )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {statCards.map((card, index) => (
                  ...
                  >
                    ...
                    <p className="mt-3 text-3xl font-black text-slate-900">{card.value.toLocaleString()}</p>
                    <p className="mt-1 text-xs text-slate-600">{card.note}</p>
                  </Link>
                ))}
              </div>

              <section className="rounded-2xl border border-indigo-100 bg-gradient-to-b from-indigo-50/60 to-white p-4 shadow-sm">
                <div className="mb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                  <div>
                    <h2 className="text-base font-bold text-indigo-900">User-wise Conversion Report</h2>
                ...
                {analyticsLoading ? (
                  <div className="h-56 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-slate-500">Loading user-wise report...</p>
                    </div>
                  </div>
                ) : userConversion.length === 0 ? (
                  <div className="h-56 rounded-xl border border-dashed border-slate-300 bg-slate-50 flex items-center justify-center">
                    <p className="text-sm text-slate-500">No report data found for selected filters.</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto border border-indigo-100 rounded-xl bg-white">
                    <table className="w-full min-w-[640px]">
                      <thead className="bg-gradient-to-r from-indigo-100 to-cyan-100 border-b border-indigo-200">
                        <tr>
                          <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-indigo-800">User</th>
                          <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-indigo-800">Total Leads</th>
                          <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-indigo-800">Converted</th>
                          <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-indigo-800">Conversion Ratio</th>
                        </tr>
                      </thead>
                      <tbody>
                        {userConversion.map((row) => (
                          <tr key={row.userId} className="border-b border-indigo-50 hover:bg-indigo-50/50">
                            ...
                            <td className="px-4 py-3 text-right text-sm font-semibold text-slate-800">{row.totalLeads}</td>
                            <td className="px-4 py-3 text-right text-sm font-semibold text-slate-800">{row.convertedLeads}</td>
            ...
            <aside className="xl:col-span-4 space-y-4">
              <section className="rounded-2xl border border-emerald-100 bg-gradient-to-b from-emerald-50/70 to-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-emerald-900">Recent Follow-ups</h3>
                  <Link to="/followups" className="text-[11px] font-semibold text-emerald-700 hover:text-emerald-900 uppercase tracking-wide">
                    View all
                  </Link>
                </div>
                ...
                <div className="space-y-2">
                  {recentFollowups.length > 0 ? (
                    recentFollowups.map((followup, idx) => (
                      <div key={idx} className="rounded-lg border border-emerald-100 bg-white p-2.5">
                        <div className="flex items-center justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-sm font-semibold text-slate-800 truncate">Follow-up #{idx + 1}</p>
                            <p className="text-xs text-slate-500 mt-0.5">Updated in activity queue</p>
                          </div>
                    ...
                    <p className="text-center py-6 text-sm text-slate-500">No recent follow-ups</p>
                  )}
                </div>
              </section>
            </aside>
