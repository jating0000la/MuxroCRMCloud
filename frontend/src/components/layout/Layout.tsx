      ...
      <div className="mx-auto max-w-6xl px-2 sm:px-4">
        <div className="flex items-center justify-center gap-1 sm:gap-2 overflow-x-auto py-2">
          ...
          return (
            ...
            >
              <span className={isActive ? 'text-primary-600 dark:text-primary-400' : 'text-slate-400 dark:text-gray-500'}>{item.icon}</span>
      ...
      <div className="flex-1 flex flex-col transition-all duration-300">
        ...
        <header className="sticky top-0 z-20 border-b border-primary-100/90 bg-white/90 backdrop-blur-xl dark:border-gray-700/90 dark:bg-gray-900/90">
          ...
          <div className="mx-auto max-w-7xl px-3 sm:px-4 py-2.5">
            <div className="rounded-xl border border-primary-100/80 bg-white/80 px-3 py-2 shadow-sm dark:border-gray-700/80 dark:bg-gray-800/80">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5 min-w-0">
                  {branding.appLogoUrl ? (
                    <img
                      ...
                      alt="App logo"
                      className="h-8 max-w-[120px] rounded-lg object-contain border border-primary-100 bg-white dark:border-gray-600 dark:bg-gray-700"
                    />
                  ) : (
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase tracking-[0.14em] text-slate-500 font-semibold truncate dark:text-gray-400">{branding.appName}</p>
                    <h2 className="text-sm sm:text-base font-bold text-slate-800 tracking-tight truncate dark:text-gray-100">
                ...
                <div className="flex items-center gap-2">
                  <div className="hidden sm:flex items-center rounded-lg border border-primary-100 bg-white p-1 dark:border-gray-600 dark:bg-gray-700">
                    <button
                      type="button"
                      onClick={() => setDensity('compact')}
                      className={`px-2 py-1 text-[11px] font-semibold rounded ${density === 'compact' ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-300' : 'text-slate-500 hover:bg-slate-100 dark:text-gray-400 dark:hover:bg-gray-600'}`}
                    >
                      Compact
                    </button>
                    <button
                      ...
                      className={`px-2 py-1 text-[11px] font-semibold rounded ${density === 'comfortable' ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-300' : 'text-slate-500 hover:bg-slate-100 dark:text-gray-400 dark:hover:bg-gray-600'}`}
                    >
                      Comfortable
                    </button>
                  </div>
                  ...
                  <button
                    onClick={toggleTheme}
                    className="flex items-center justify-center w-10 h-10 rounded-lg border border-primary-100 bg-white hover:bg-primary-50/70 transition-colors dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600"
                  ...
                  <div className="relative" ref={notificationMenuRef}>
                    <button
                      ...
                      className={`relative flex items-center justify-center w-10 h-10 rounded-lg border bg-white transition-colors dark:bg-gray-700 ${
                    ...
                    >
                      <svg className={`w-5 h-5 ${notifications.some(n => n.type === 'overdue') ? 'text-red-600' : notifications.some(n => n.type === 'today') ? 'text-amber-600' : 'text-slate-600 dark:text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="relative" ref={userMenuRef}>
                  <button
                    ...
                    className="flex items-center space-x-2 rounded-lg border border-primary-100 bg-white px-2 py-1.5 hover:bg-primary-50/70 dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600"
                    aria-label="User menu"
                    aria-expanded={userMenuOpen}
                  >
                    <div className="w-7 h-7 bg-primary-100 rounded-full flex items-center justify-center dark:bg-primary-900/40">
                      <span className="text-xs font-bold text-primary-700 dark:text-primary-300">
                        {user?.name?.charAt(0)?.toUpperCase()}
                      </span>
                    </div>
                    <div className="hidden sm:block text-left leading-tight">
                      <div className="text-xs font-semibold text-slate-700 truncate max-w-[140px] dark:text-gray-200">{user?.name}</div>
                      <div className="text-[11px] text-slate-400 dark:text-gray-400">{user?.role}</div>
                    </div>
                    <svg
        ...
        <footer className="px-4 pb-20 text-center text-xs text-slate-500 dark:text-gray-500">
