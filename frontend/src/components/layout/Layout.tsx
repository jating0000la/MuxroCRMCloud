              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="relative" ref={notificationMenuRef}>
                    ...
                    >
                      <svg className={`w-4 h-4 ${notifications.some(n => n.type === 'upcoming') ? 'text-blue-600' : 'text-slate-600 dark:text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                      </svg>
                      {pendingCount > 0 && (
                        <span className={`absolute -top-1 -right-1 flex items-center justify-center min-w-[16px] h-4 px-1 text-[10px] font-bold text-white rounded-full ${notifications.some(n => n.type === 'upcoming') ? 'bg-blue-500 animate-pulse' : 'bg-amber-500'}`}>
                          {pendingCount}
                        </span>
                      )}
                    </button>
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div className="min-w-0 flex-1">
                                      <div className={`text-xs font-bold uppercase tracking-wide ${notif.type === 'upcoming' ? 'text-blue-700' : 'text-amber-700'}`}>
                                        {notif.type === 'upcoming' ? 'Upcoming (10 min)' : 'Follow-up'}
                                      </div>
